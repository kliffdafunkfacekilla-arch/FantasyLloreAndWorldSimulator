...
public:
    ...
    void RunThermalErosion(WorldBuffers& buffers, const NeighborGraph& graph, int iterations, uint32_t count);
};

class HydrologySim {
public:
    void CalculateFlow(WorldBuffers& buffers, const NeighborGraph& graph, uint32_t count);
};

class ClimateSim {
public:
    void UpdateGlobalClimate(WorldBuffers& buffers, const WorldSettings& settings);
    void GenerateBiomes(WorldBuffers& buffers, WorldSettings& settings, uint32_t count);
};

class LifeSim {
public:
    void SeedSpecies(WorldBuffers& buffers, const AgentTemplate& dna, uint32_t startCell);
...
class AgentSystem {
public:
    void TickAgents(WorldBuffers& buffers, std::vector<AgentTemplate>& registry, float deltaTime, uint32_t count);
private:
    void HandleConstruction(uint32_t cellID, WorldBuffers& buffers, AgentTemplate& dna);
    void HandleBorderExpansion(uint32_t cellID, WorldBuffers& buffers, AgentTemplate& dna);
};
...
public:
    ...
    void RecordEvent(const ChronosConfig& time, std::string cat, int cellID, std::string text);
...
public:
    void ResolveBorders(WorldBuffers& b, const std::vector<AgentTemplate>& registry, 
...
public:
    ...
    void GlobalChronosTick(WorldBuffers& buffers, ChronosConfig& time, float realDeltaTime);
