...
void UpdateGlobalClimate(WorldBuffers &b, const WorldSettings &s) {
  for (uint32_t i = 0; i < b.count; ++i) {
    float y = b.posY[i];
    float latEffect = (s.worldScaleMode == 3)
                          ? (1.0f - std::abs(y - 0.5f) * 2.0f)
                          : (1.0f - y);
    float elevationChill = b.height[i] * 0.5f;
    if (b.temperature)
      b.temperature[i] = (latEffect - elevationChill) * s.globalTempModifier;
...
// --- 3. Visualizer Bridge ---
void UpdateVisualBuffers(WorldBuffers &b, const WorldSettings &s,
                         MapRenderer &r) {
  for (uint32_t i = 0; i < b.count; ++i) {
    uint32_t idx = i * 3;
    float h = b.height[i];
    ...
    if (h < s.seaLevel) {
      r.colorBuffer[idx + 0] = 0.1f;
      r.colorBuffer[idx + 1] = 0.2f;
      r.colorBuffer[idx + 2] = 0.6f;
    } else if (b.temperature && b.temperature[i] < 0.2f) {
      r.colorBuffer[idx + 0] = 0.9f;
      r.colorBuffer[idx + 1] = 0.9f;
      r.colorBuffer[idx + 2] = 1.0f;
    } else {
      if (b.factionID && b.factionID[i] > 0) {
        ...
        r.colorBuffer[idx + 0] = 0.2f;
        r.colorBuffer[idx + 1] = 0.5f + (h * 0.2f);
        r.colorBuffer[idx + 2] = 0.2f;
      }
    }
  }
...
// --- UI Stub ---
void DrawLoreUI(LoreScribe &scribe, TimelapseManager &tl,
                uint32_t currentCenterCell) {
  // Requires ImGui. Code commented to avoid build error but structure
  // preserved.
  /*
