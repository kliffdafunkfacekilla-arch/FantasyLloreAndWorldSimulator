...
void MemoryManager::InitializeWorld(const WorldSettings &settings,
  ...
  if (settings.enableClimate) {
    ...
    buffers.windDY =
        new float[count]; // Added DY for wind visualization if needed
  ...
  if (settings.enableFactions || settings.resourceCount > 0) {
    ...
    if (settings.resourceCount > 0) {
      ...
      std::memset(buffers.resourceLevels, 0,
                  count * settings.resourceCount * sizeof(float));
    }
