...
void SimulationLoop::Update(WorldBuffers& buffers, std::vector<AgentTemplate>& registry, 
    ...
    if (time.dayCount % 2 == 0) {
         conflictSystem.ResolveBorders(buffers, registry, factions, graph, scribe, time, dt);
    }
    
    // 5. Timelapse Snapshot (Monthly)
    // Assuming 30 days per month
    // If dayCount wraps, we need to check cycle. 
    // In ChronosSystem logic, dayCount resets. So if dayCount == 0, it's new month.
    if (time.dayCount == 0) {
