...
void TimelapseManager::TakeSnapshot(const WorldBuffers &b,
                                    const ChronosConfig &time) {
  ...
  // Rough estimation
  if (b.count > 0)
    side = (int)std::sqrt(b.count);
  else
    return;

  if (taggedCellStart >= b.count)
    return;
    ...
    for (int x = -radius; x < radius; ++x) {
      ...
      if (targetX >= 0 && targetX < side && targetY >= 0 && targetY < side) {
        ...
        if (target >= 0 && target < (int)b.count) {
          if (b.factionID)
            s.factionData.push_back(b.factionID[target]);
          if (b.population)
            s.popData.push_back((float)b.population[target]);
        }
      }
    }
  }
